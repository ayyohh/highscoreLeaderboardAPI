...
var GameState = {
  ...
  preload: function() {
      this.load.image('ground', 'http://ayyohh.net/escapefromunionstation/images/union.png');
      this.load.image('platform', '../../../images/train.png');
      this.load.image('goal', '/weedstore.png');
      this.load.image('arrowButton', 'images/arrowButton.png');
      this.load.image('actionButton', 'images/actionButton.png');
      this.load.image('copart', 'images/copart.png');

      this.load.spritesheet('player', 'images/ninja.png', 28, 30, 5, 1, 1);
      this.load.spritesheet('fire', 'images/fire_spritesheet.png', 20, 21, 2, 1, 1);

      this.load.text('level', 'images/level.json');
  },
  ...
  win: function(player, goal) {
    alert('You survived Union Station in: ' + endTime.toFixed(3) + ' seconds!!!');
    $('#leaderBoardModal').modal('show');
    game.state.start('GameState');
  ...
  createBarrel: function() {
    ...
    var copart = this.barrels.getFirstExists(false);

    if(!copart) {
      copart = this.barrels.create(0, 0, 'copart');
    }

    copart.body.collideWorldBounds = true;
    copart.body.bounce.set(1, 0);

    copart.reset(this.levelData.goal.x, this.levelData.goal.y);
    copart.body.velocity.x = this.levelData.barrelSpeed;
  }
